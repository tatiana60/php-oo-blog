...
if(isset($_POST['category_create'])) {
    ...
    $info = $repository->update($category);

    if($info == 1) {
        header('Location: /admin/category/read.php?id='.$id);
    } else {
        http_response_code(403);
        exit();
    }
...
<div class="container-fluid">
    <div class="row">
        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
            ...
            <!-- TODO Add content -->
            <form method="POST" action="/admin/category/update.php?id=<?= $category->getId() ?>">
                <div class="form-group row">
                    <label for="name" class="col-sm-2 col-form-label">Nom</label>
                    <div class="col-sm-10">
                        <input type="text" class="form-control" id="name" name="name" value="<?= $category->getTitle() ?>">
                    </div>
                </div>
                <div class="form-group row">
                    <div class="col-sm-10 offset-sm-2">
                        <button name="category_create" type="submit" class="btn btn-primary">Enregistrer</button>
                    </div>
                </div>
            </form>
